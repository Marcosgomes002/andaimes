...
import { dadosEmpresaInicial } from '@/data/mockData';
...
interface AppState {
  ...
  atualizarAluguel: (id: string, params: any) => Promise<void>;
...
function mapProductToProduto(p: any): Produto {
  return {
    ...
    quantidade_total: Number(p.total_quantity ?? 0),
    quantidade_disponivel: Number(p.available_quantity ?? 0), 
    quantidade_alugada: Number(p.rented_quantity ?? 0),
    ativo: p.active ?? true, 
